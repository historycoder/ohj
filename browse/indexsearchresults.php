...
$statement = "SELECT * FROM " . $dbcon['journal_db_table'] . " WHERE ". $criteria .  " LIKE '%" . $text . "%' AND Year<>2000 ";
$pages = new Paginator;
...
echo( '<div style="width:100%" class="container">' );
	...
	while($row = $results->fetch(PDO::FETCH_ASSOC)) {
		if($row['Subject'] != $subject){
			...
			echo "<br><dd>";
			if(strlen(trim($row['Title'])) AND strlen(trim($link))){
			...
			if(strlen(trim($link))){
				...
				$newtitle = str_replace("See Also",$row['Title'], "<i><b>See Also</b></i>");
				echo $newtitle;
				#echo "<br><dd>" . str_replace("See Also",$row['Title'], "<i><b>See Also</b></i>");
			}
			if(strlen(trim($row['Author']))){
				echo $row['Author'];
			}
			if(((strlen(trim($newstartpage))) || (strlen(trim($row['StartPage']))))  AND strlen(trim($link))){
				echo "<dd><a href=\"displaypages.php?display[]=".$newvolume."&display[]=".$newstartpage."&display[]=".$newendpage."\">";
			}
			if(strlen(trim($newstartpage))){
				echo trim($newstartpage)."</a>";
